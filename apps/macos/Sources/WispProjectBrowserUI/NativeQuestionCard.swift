...
struct NativeQuestionCard: View {
    ...
    private var enabled: Bool { conversation.canStageQuestion(target) }
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(question.title).font(WispDesign.font(size: 14)).textSelection(.enabled)
            if pending {
                ForEach(Array(question.options.enumerated()), id: \.offset) { _, option in
                    Button { conversation.stageQuestionAnswer(option.answer, target: target) } label: {
                ...
                if question.allowFreeform {
                    HStack {
                        ...
                        Button("填入回答") {
                            if conversation.stageQuestionAnswer(freeform, target: target) { freeform = "" }
                        }.disabled(!enabled || freeform.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
                    }.disabled(!enabled)
                }
                Text(question.requestID == nil ? "回答会加入输入框并保留已有草稿，发送后继续。" : "该问题需要 ACP 实时回复，请在支持的客户端继续。")
