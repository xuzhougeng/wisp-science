...
如果希望多个订阅型 ACP Agent 参与同一份可见讨论，请保留普通 Wisp 会话，
在编辑器中输入 `@` 并选择一个已配置的 **ACP Agent**。每条消息只能点名一个
参与者；Wisp 会为每个 profile 维护独立的隐藏 ACP 子会话，将它尚未看到的父
会话增量作为上下文发送，并把回复连同 Agent label、profile ID 和 ACP session
provenance 回写到父会话。首版不包含 `@all`、自动 roundtable 或 Agent 主动互相
点名。
...
在桌面对话中，可在光标处手动输入 `@` 点名已配置的 ACP Agent，或附加已保存的
产物、上传的文件、执行环境和语言运行时；输入 `#` 附加已保存的会话（包括其他
项目的会话），输入 `/` 让下一轮使用已启用的 skill。这些选择器在编辑消息的
任意位置都可使用，粘贴文本时不会弹出。附件会显示为可移除的显式标签；跨项目
产物保留原始本地路径，不会被自动复制。ACP Agent 同样支持这些引用：所选 skill
与会话上下文作为 ACP 文本块发送，产物则作为文件链接发送。
