...
fn install_runtime_poll(status: RwSignal<PetStatus>, activity: RwSignal<DesktopPetActivity>) {
    // Fast poll while the pet is enabled (it mirrors live run activity), slow
    // probe while disabled: the poll still notices the user turning the pet
    // back on within a few seconds, but a pet-less session stops paying three
    // IPC round-trips (two settings reads + one SQLite run query) every 2 s.
    let mut idle_polls: u32 = 0;
    let callback = Closure::wrap(Box::new(move || {
        let fast = status.get_untracked().enabled && idle_polls < 2;
        idle_polls = if fast {
            0
        } else {
            idle_polls.saturating_add(1)
        };
        refresh_desktop_pet_fast(fast, status, activity);
    }) as Box<dyn FnMut()>);
