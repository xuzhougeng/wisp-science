...
- Each registered context can open an ephemeral interactive terminal in a separate Wisp window. Local, WSL, and system-OpenSSH launch adapters share one PTY-backed TerminalSession manager; closing the view detaches it, while explicit termination ends the process. Terminal buffers are not persisted and tracked computation continues to use Runs.
