...
## 当前阶段状态（2026-09-26，不含 ACP）

按用户重新限定的范围，本阶段只做可靠性收尾与实机验收。起点为 main `f684c799`，实施分支 `codex/native-reliability-phase2`。以下历史 ACP 和长期 parity 事项继续保留，不属于本阶段验收门槛。

- 同步冲突闭环已通过稳定包 CUA：确认/立即 Escape/取消、缺失快照失败后显式重试、不同内容下保留本地与采用远端；快照内容和父节点经独立检查。不是实际双设备/云盘验证。
- 旧格式目录已在两端导入且历史可读；原生目录选择器第一次立即 Escape 只关闭选择器，父窗口保留。无效测试 fixture 的根会话字段已修正，未修改产品代码掩盖测试数据问题。
- 快速粘贴发送 6 次、两种换行/发送偏好、会话切换和关闭最后窗口后的草稿保留通过；普通工具审批等待/会话切换/反馈 Escape/拒绝后的草稿也已通过；搜狗候选确认由用户完成两种偏好下的人工操作并反馈未发送，工具/请求记录核对保留文本及单次显式发送。
- 两端 QA 包均为 clean `82256473`（仅文档区别于产品基线）；Swift 全套 289 passed、格式及标准构建/签名通过。Rust 全套 2361 passed，wasm check 通过，Playwright 870 passed、2 skipped；本次稳定源码的自动化回归已收齐。
- 已保存同一 35-turn fixture 和 16 张条件配对截图，记录各 3 次切换、首页打开、滚动，以及限定主进程/helper 范围的 idle CPU。冷启动/帧率/完整 WebKit 进程 CPU 未量化，不宣称性能胜负。解锁后的协作 IME 补验也已完成，本轮四项验收证据已收齐；候选栏前提依赖用户反馈，未宣称全自动捕获。

新增观察：英文偏好下原生工作区仍有中文标签，原生会话正文显示 Usage JSON；两者保留为后续独立修复，不扩展 ACP。

实际步骤、原始数据、可复用合成 fixture 和待验门槛见 [第二阶段验收记录](../../design-qa/native-reliability-2026-09-26/README.md)。

## PR 汇总状态（2026-09-25，历史）
