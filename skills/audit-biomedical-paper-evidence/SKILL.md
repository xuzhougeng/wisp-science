...
1. 已取得用户提供或经授权检索并核对的至少一项可读目标材料：PDF、摘要、正文、figure、panel、caption 或截图；
2. 请求涉及科学问题、实验设计、逐图解读、机制链、因果性、rescue、方法比较、证据强度或研究局限；
3. 当前材料足以支持相应层级的判断。

若只有标题或 DOI，先进入材料获取前置流程，不直接审读全文。用户要求“找全文并解读”即包含检索授权；
未授权检索时先请求材料或询问是否检索。有检索工具时按 `references/pdf-retrieval-fallback.md` 获取并核对材料；
公开渠道未找到可获取的 PDF 后，可通过卡片选择科研通求助，并在核实积分、获得提交确认后提交。
用户提供或经授权取得的可读材料均可用于审读，不得声称已读取尚未取得的全文。医学诊断、治疗选择、个体化用药、可直接执行的湿实验步骤、
...
审读前至少取得一项用户提供或经授权检索并核对的目标论文材料。若用户未指出具体问题，先从可见材料中识别：
...
12. **选择并导出**：审读内容完成后，按 `references/export-and-diagrams.md` 调用 Wisp 的 `ask_user` 卡片选择格式并确认模型视觉能力；等待用户回答后才导出，不默认生成 Markdown 文件。
...
- 导出前按参考文件渲染图像并用视觉工具检查；卡片确认不能代替实际图像检查。
...
- 只有标题或 DOI：暂停全文审读，按材料获取前置流程检索或请求材料；科研通求助待应助不等于已取得 PDF。
...
- 审读完成后的格式选择卡片、视觉确认、Mermaid 渲染与 Word 排版：`references/export-and-diagrams.md`
- 只有标题/DOI 时的授权检索、停止条件、科研通登录与积分提交卡片：`references/pdf-retrieval-fallback.md`
